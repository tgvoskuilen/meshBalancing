...
Description
    ...
    settings in the balanceParDict file.
...
)
{
    ...
    (
        ...
        mesh,
        dimensionedVector(name, dimless, vector::zero)
    );
...
label topParentID(label p, const refinementHistory& history)
{
    label nextP = history.splitCells()[p].parent_;
    if( nextP < 0 )
    {
        return p;
    }
    ...
    {
        return topParentID(nextP, history);
    }
}

label log2Func(label x)
{
    label p = -1;
    while(x)
    {
...
#   include "createTime.H"
    ...
    forAll(mesh.cells(), cellI)
    {
        ...
        {
            uniqueIndex[cellI] = mesh.nCells() + topParentID
            (
    ...
    // Create decompositionMethod and new decomposition
    {
        ...
        finalDecomp = decomposer().decompose
        (
            mesh, 
            localIndex,
            coarsePoints,
